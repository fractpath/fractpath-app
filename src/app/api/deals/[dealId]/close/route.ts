...
export async function POST(
  ...
  try {
    ...
    if (deal.status !== "FUNDED") {
      return jsonError(
        `Invalid transition: ${deal.status} → CLOSED`,
        400,
      );
    }
    ...
    if (updateError)
      return jsonError("Failed to close deal", 500, {
    ...
    if (eventErr) {
      console.error("deal_closed_event_insert_error", eventErr.message);
      return NextResponse.json({
        ok: true,
        warning: "Closed, but failed to write audit event",
    ...
    console.error("close_route_fatal", e?.message || e);
