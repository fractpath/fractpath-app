...
export async function POST(
  ...
  try {
    ...
    if (deal.status !== "ACCEPTED") {
      return jsonError(
        `Invalid transition: ${deal.status} → EXECUTED`,
        400,
      );
    }
    ...
    if (updateError)
      return jsonError("Failed to execute deal", 500, {
    ...
    if (eventErr) {
      console.error("deal_executed_event_insert_error", eventErr.message);
      return NextResponse.json({
        ok: true,
        warning: "Executed, but failed to write audit event",
    ...
    console.error("execute_route_fatal", e?.message || e);
