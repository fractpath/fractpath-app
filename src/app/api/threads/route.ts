...
/**
 * NOTE: Sprint 13 canonical offer surface is /api/deals/[dealId]/submit-offer.
 * This legacy endpoint is kept only for backward compatibility, but it must
 * NEVER create orphan deal_threads rows without deal_id.
 */
export async function POST(request: NextRequest) {
  ...
  if (!UUID_RE.test(dealId)) {
    return jsonError("deal_id must be a valid UUID", 422);
  }
