...
export async function POST(
  ...
  const isOwner = deal.owner_user_id === user.id || userRole === "OWNER";
  const isCounterparty = userRole === "COUNTERPARTY";

  if (!isOwner && !isCounterparty) {
    return jsonError("Forbidden (OWNER or COUNTERPARTY only)", 403);
  }
  ...
  if (insertError || !versionRow) {
    ...
    return jsonError("Failed to create counter version", 500);
  }

  const { error: eventError } = await (service.from("deal_events") as any).insert({
    deal_id: dealId,
    event_type: "DEAL_COUNTER_CREATED",
    payload: {
