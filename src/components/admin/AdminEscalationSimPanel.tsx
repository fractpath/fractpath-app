  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-md bg-amber-50 border border-amber-200 px-3 py-2.5 text-xs text-amber-800 space-y-0.5">
        ...
        <div>
          Real Stripe payment collection and ATTOM valuation ordering are not connected.
          This section simulates escalated deposit and valuation states without affecting
          homeowner-facing views or live payment systems.
        </div>
      </div>
      ...
      <div className="rounded-md border px-4 py-3 space-y-2.5">
        <div className="flex items-center gap-2 flex-wrap text-sm font-medium">
          <span>Step 1 — Escalation deposit</span>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {depositStatus === "failed" && (
            <p className="text-xs text-red-700">
              Deposit failed. Reset to retry the simulation.
            </p>
          )}
          {depositStatus && (
            <button
      ...
      >
        <div className="flex items-center gap-2 flex-wrap text-sm font-medium">
          <span>Step 2 — Stronger valuation (ATTOM placeholder)</span>
          {/* TODO(attom): Replace with live ATTOM order placement + result ingestion */}
          {avmStatus ? (
            <span
              className={`text-xs rounded-full px-2 py-0.5 font-normal ${
          ...
          {!depositPaid && (
            <span className="text-xs text-muted-foreground italic">(requires deposit paid)</span>
          )}
        </div>
        ...
        {avmComplete && (
          <div className="rounded-md bg-green-50 border border-green-200 px-3 py-2 text-xs text-green-800">
            Escalated FMV applied to this property&apos;s verified basis (source:{" "}
            <span className="font-mono">escalated_sim</span>). Deal-term eligibility
