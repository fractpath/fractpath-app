...
- **Deal Header Persistence:** `DealHeader` persists title + property to server via `PATCH /api/deals/[dealId]/header` (stores as `DEAL_HEADER_UPDATED` deal_event). Deal page fetches latest header event server-side and passes to `DealHeader` as initial props, ensuring shared/read-only mode displays persisted title + property without relying on localStorage.
