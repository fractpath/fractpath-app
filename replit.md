...
- **Public Property Map (Phase 1):** `/map` page with MapLibre GL JS (v5) rendering Mapbox Streets raster tiles via `NEXT_PUBLIC_MAPBOX_TOKEN`. `GET /api/map/public-properties` returns enriched `DiscoveryProperty[]` (id, address, lat, lng, status, verified_at, hero_photo_url, latest_verified_fmv, beds, baths, sqft, year_built, property_type) for verified+public+non-private properties with lat/lng. Hero photo: owner hero → first owner photo → Mashvisor cover fallback. Markers rendered as HTML elements; click opens popup with thumbnail, address, facts, FMV, and "View Property" link. Default center: Annapolis, MD (38.978, -76.492), zoom 8. No auth required.
- **Verified Properties Discovery Surface:** `/verified-properties` refactored into a map+cards discovery page. Server component fetches enriched `DiscoveryProperty[]` (including `latitude`, `longitude`, `latest_verified_fmv` from properties table). Passes to `VerifiedPropertiesClient` (client component) which renders: search input (city/ZIP/address), sort control (newest/value-desc/value-asc), `PropertyMapEmbed` (shared data — no separate API fetch), and property cards grid. Marker click → highlights card and scrolls to it; card click → highlights card and flies map to that marker. Cards show est value from `properties.latest_verified_fmv` (not Mashvisor `value_estimate`). Properties without lat/lng appear in card grid but not on map. `PropertyMapEmbed` is a stable map component (never remounts on filter/sort changes; markers synced incrementally via useEffect; bounds fit on first load only).
