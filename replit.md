...
- **Deal Header Persistence (canonical model):** One canonical source for deal property identity: `DEAL_HEADER_UPDATED` events in `deal_events`, written exclusively by `PATCH /api/deals/[dealId]/header` (DealHeader's PropertyForm onResolved and EditDealNameModal onSaved). Snapshots embed `meta.header` from the canonical event at compute time (server reads latest event, never trusts client-sent header). Dashboard reads events as primary, `snapshot_json.meta.header` as fallback. localStorage is NOT a source of truth — used only for ephemeral in-session state within DealHeader component. New deals created via `POST /api/deals/create` start as empty shells (no snapshot, no header event, no terms). Client components (DealDetailWidgetPanel, RecomputeSnapshotButton) never send header data to compute endpoints.
- **Deal Page (buyer/participant path):** `src/app/deal/[dealId]/page.tsx` first loads deal via user's Supabase client (RLS-gated by `deal_access_grants`). If found, renders full workspace: `DealHeader` (with server-fetched title/property from DEAL_HEADER_UPDATED events), `DealDetailWidgetPanel` (latest snapshot), `DealActivityFeed` (deal events). Owner fallback path (via thread+property chain) still exists for property owners not yet granted direct access. Fallback also checks `thread_invites` by user email (with expiry validation) for homeowners who were invited but don't yet own the property record.
- **Feature Specifications:**
    ...
    - **Default Scenario:** Provides baseline `deal_terms` and `scenario` for new deals. `ensureScenario` merges user terms field-by-field with defaults (missing fields filled, user values always win).
    - **Recompute Button:** Enables owners to regenerate snapshots with current inputs. Rendered on deal page for owners when not locked and snapshot exists.
    - **Create Deal Flow:** Supports creation of new deals via an authenticated endpoint and dedicated page.
    - **Snapshot KPI Extractor:** Extracts display data from snapshots for UI components. FMV badge prioritizes `inputs.deal_terms.property_value` (user's stated FMV) over computed projections like `projected_fmv`.
    - **Widget Save Merge:** `DealWidgetShell.handleSave` merges widget output with existing seed snapshot terms to prevent field regression when the widget omits fields.
...
- **Calculator Widget Package:** Provides React UI components (`DealSnapshotView`, `DealEditModal`) and utilities for snapshot building. **Important:** `FractPathCalculatorWidget` does NOT read `initialSnapshot` prop — it hardcodes defaults. The deal page uses `DealSnapshotView` (display) + `DealEditModal` (editing) instead, which properly initialize from saved data.
