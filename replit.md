...
- **Deal Header Persistence:** `DealHeader` persists title + property to server via `PATCH /api/deals/[dealId]/header` (stores as `DEAL_HEADER_UPDATED` deal_event, auth via `deal_access_grants` OWNER role). Additionally, header data (title, display_address, property_id, property_status, ownership_status) is written into `snapshot_json.meta.header` on every snapshot POST/compute/create. A `DEAL_HEADER_UPDATED` event is also written on deal creation and snapshot compute when header data is present, ensuring the dashboard event-first lookup works from day one. Client-side callers (DealDetailWidgetPanel, RecomputeSnapshotButton, NewDealClient) read header from localStorage and include it in snapshot payloads. DealHeader syncs localStorage with server-provided initial state on mount (merge strategy: server property overwrites local property when present; local data preserved when server has no property). Deal page uses deal_events header as primary source (fallback: snapshot header), ensuring shared/read-only mode displays persisted title + property without relying on localStorage. Dashboard deal cards read from `DEAL_HEADER_UPDATED` events as primary source, `snapshot_json.meta.header` as fallback.
