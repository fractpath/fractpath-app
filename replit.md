...
- **Contract Versioning:** `src/lib/contractVersion.ts` defines `CONTRACT_VERSION = "11.0.0"` and `SCHEMA_VERSION = "11"`. All snapshot persistence boundaries (resume, fork, compute routes) stamp these values. The compute engine's own `compute_version` is independent and set by the `@fractpath/compute` package.
- **Calculator Widget Package:** Provides React UI components (`DealSnapshotView`, `DealEditModal`) and utilities.
- **Canonical Compute Package:** The core `@fractpath/compute` engine, serving as the single source of truth for deal calculations and versioning. Resolved via Next.js webpack alias from `fractpath-calculator-widget/packages/compute/dist`.
