...
- **Mashvisor Enrichment System:** `property_enrichments` table stores `raw_payload`, `summary_payload`, `images_payload` per property+provider. Admin-gated `POST /api/admin/properties/[propertyId]/review/fetch-mashvisor` writes current enrichment row (partial unique index on property+provider+is_current). Provider constraint now allows `'mashvisor'` and `'rentcast'`.
- **RentCast Facts Migration (Phase 2):** Property fact display (beds/baths/sqft/address/property type) on owner, public, and admin pages is now sourced from `property_review_runs.normalized_payload` (artifact_type=`property_profile`, provider=`rentcast`) via the provider-agnostic `PropertyFacts` type (`src/lib/property/propertyFacts.ts`). Mashvisor `images_payload` is preserved as the fallback image source. `EnrichedPreviewData.summary` and `.images` are now optional; new `facts?: PropertyFacts` field takes priority for fact display. `AdminMashvisorPanel` accepts `rentcastFacts?` and surfaces a "Facts source: RentCast" row in the admin metadata block. No new RentCast API calls are made at render time — all data is read from existing persisted rows.
