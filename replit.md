...
- **Public Property Map (Phase 1):** `/map` page with MapLibre GL JS (v5) rendering Mapbox Streets raster tiles via `NEXT_PUBLIC_MAPBOX_TOKEN`. `GET /api/map/public-properties` returns light JSON (id, address, lat, lng, status, verified_at, hero_photo_url) for `status=verified AND visibility_preference=public AND is_private=false AND lat IS NOT NULL AND lng IS NOT NULL`. Hero photo: owner hero photo → first owner photo → Mashvisor cover fallback. Markers rendered as HTML elements; click opens a popup card with hero thumbnail, address, and link to public property detail page. Default center: Annapolis, MD (38.978, -76.492), zoom 8. No auth required for either the page or the API route.
